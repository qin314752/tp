	...
	<div class="Hui-article">
		<article class="cl pd-20">
			...
			<div class="cl pd-5 bg-1 bk-gray mt-20">
				<span class="l"><a href="javascript:;" onclick="admin_role_add('添加角色','admin_role_add','800')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加角色</a> </span>
			</div>
...
function admin_stop(obj,id,status){
	...
	layer.confirm('确认要修改吗？',function(index)
	{
		...
		var id1 = id;
		var status1 = status;
		$.get("__URL__/role_suop",{id:id1,status:status1},function(data){
		if(data==1){
...
function admin_role_del(obj,id){
	layer.confirm('角色删除须谨慎，确认要删除吗？',function(index){
		//此处请求后台程序，下方是成功后的前台处理……
		var role_id = id;
		$.get("__URL__/role_del",{id:role_id},function(data){
			if(data == 1){
				window.location.reload()
